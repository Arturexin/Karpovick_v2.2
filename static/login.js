...
registro.addEventListener('submit',async (event)=>{
    ...
    if(document.getElementById("password-create").value === document.getElementById("password-confirmar-create").value &&
        ...
        if(response.ok){
            alert("Usuario creado correctamente, se le enviará un mensaje de confirmacion a su correo electrponico.")
...
/* document.getElementById('formulario-create').addEventListener('submit', function(event) {
   event.preventDefault();

   btn.value = 'Enviando...';

   const serviceID = 'default_service';
   const templateID = 'template_c5ey4us';

   emailjs.sendForm(serviceID, templateID, this)
    .then(() => {
      btn.value = 'Registrar Cuenta';
      alert('Su solicitud ha sido recepcionada, se le enviará un correo de confirmación al Email que nos proporcionó.');
    }, (err) => {
      btn.value = 'Registrar Cuenta';
